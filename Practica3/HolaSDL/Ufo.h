...
public:
	Ufo() = default;
	Ufo(Point2D<> position, Texture* texture, PlayState* playState, bool direction, int state);
	Ufo(Point2D<> position, Texture* texture, PlayState* playState, bool direction, int state, int hT);
